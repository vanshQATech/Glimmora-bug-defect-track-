...
async function sendPasswordResetEmail({ to, resetLink }) {
  const transport = getTransporter();
  if (!transport) {
    console.error(`[Mailer] SMTP not configured. Cannot send password reset to ${to}.`);
  ...
  try {
    ...
    console.error(`[Mailer] SMTP verify failed for password reset:`, err.message);
    transporter = null;
    return { success: false, error: `SMTP connection failed: ${err.message}` };
  }
  ...
  try {
    const info = await transport.sendMail({
      from: process.env.SMTP_FROM || process.env.SMTP_USER,
      to,
      subject: 'Reset your Glimmora DefectDesk password',
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 20px;">
          <div style="text-align: center; margin-bottom: 32px;">
            <h1 style="color: #4f46e5; font-size: 24px; margin: 0;">Glimmora DefectDesk</h1>
          ...
          <div style="background: #ffffff; border: 1px solid #e5e7eb; border-radius: 12px; padding: 32px;">
            ...
            <div style="text-align: center;">
              <a href="${resetLink}" style="display: inline-block; background: #4f46e5; color: #ffffff; text-decoration: none; padding: 12px 32px; border-radius: 8px; font-size: 14px; font-weight: 600;">
                Reset Password
              </a>
            </div>
            <p style="color: #9ca3af; font-size: 12px; margin-top: 24px; text-align: center;">
